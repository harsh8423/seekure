...
export default function JobCard({ job }) {
  ...
  return (
    <div className="w-full bg-white rounded-xl hover:shadow-lg md:hover:shadow-2xl transition-all duration-300 border border-gray-100 group">
      ...
      <div className="px-4 md:px-6 py-3 md:py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-t border-gray-100">
        <div className="text-xs md:text-sm text-gray-500 flex items-center gap-1 flex-shrink-0">
          ...
          <span>Posted {formatRelativeDate(job.date_posted)}</span>
        </div>
        <div className="flex flex-wrap gap-2 w-full sm:w-auto justify-end">
          <button
            onClick={() => setIsDetailsOpen(true)}
            className="px-2 md:px-3 py-1 md:py-1.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-xs md:text-sm flex items-center gap-1"
